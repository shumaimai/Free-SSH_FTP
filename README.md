...
- **sudo パスワード入力支援**: `[sudo] password for ...` などのプロンプトを検知し、確認後に保存済みパスワードを送信(既定 OFF)。リモート側はプロンプトを偽装できるため、確認なしの自動送信は行わない
...
## 設定(ファイル → 設定)
sudo プロンプト検知 / 右クリック貼り付け / 権限無視の既定 / 内蔵エディタで開く / ターミナル・エディタの文字サイズ・タブ幅。
