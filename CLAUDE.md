...
- **最新リリース: v0.8.0**(2026-07-25)。`main` = `0bdfffe` 時点でオープン PR は 0 件。
- テストは **436 passed, 2 skipped**(`QT_QPA_PLATFORM=offscreen pytest`)。
...
| # | 内容 | 状態 |
|---|---|---|
| **#129** | Windows 実機の通し検証 | 実マウス D&D 等の手動検証項目が残るためオープン維持 |
| **#82** | ローカル⇔リモートのデュアルペイン(WinSCP 流。目玉機能) | **第 1 段 + 第 2 段(同期ブラウズ)を実装済み。** 実機確認待ちで close 可否はオーナー判断 |
| **#65** | クラウド同期のバックエンド再検討 | **オーナー合意待ち。勝手に実装しない。** 再開時は Firebase 案が本命 |
...
26. **リフロー第 2 段(#100)の不変条件**: リサイズしても**カーソル行の開始位置は
    動かさない**(PTY 側のカーソル把握とずれる)。履歴行の継続フラグ/幅は
    行オブジェクトの属性(`hashi_wrapped` / `hashi_cols`)として `index()` が刻む。
    prev/next_page で行オブジェクトがそのまま移動するため、位置ベースの集合へ
    置き換えないこと。
27. **sshd の reload は既存接続を切らない方法で**。systemd は `systemctl reload`、それ以外は
