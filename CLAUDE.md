...
6. **右クリック貼り付け**: 右クリック=貼り付け(PuTTY 流)。メニューは **Shift+右クリック**。
   左で選択したら即コピー。
7. **パスワード入力支援**: プロンプト検知は保守的な正規表現(`terminal._PW_PATTERNS`)。
   リモート側は sudo プロンプトを偽装できるため、**確認なしの自動送信は禁止**。設定が ON でも
   送信前に確認し、password/passphrase は手動送信のみ。誤りループ防止に **8 秒クールダウン**。
   手動送信は右クリックメニュー(→ `password_prompt.emit("manual")`)。
